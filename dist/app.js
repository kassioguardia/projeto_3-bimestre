...
function atualizarCards(relatorio) {
    if (relatorio.length === 0)
        return;
    const totalClientes = relatorio.length;
    const somaValores = relatorio.reduce((acumulador, item) => {
        return acumulador + Number(item.valor_total);
    }, 0);
    const mediaValor = somaValores / totalClientes;
    const clienteMaisVendas = relatorio.reduce((maior, item) => {
