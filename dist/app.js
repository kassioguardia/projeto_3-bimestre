...
import { formatCurrency, safeNumber } from './utils/dom.js';
export async function carregarDashboard() {
    try {
        ...
        if (dashboard) {
            const alertEl = dashboard.querySelector('.dashboard-error-alert');
            if (alertEl) {
...
function atualizarCards(relatorio) {
    ...
    if (!Array.isArray(relatorio) || relatorio.length === 0) {
        ...
        if (elMedia)
            elMedia.textContent = formatCurrency(0);
        if (elMaiorCliente)
    ...
    const vendasPorCliente = relatorio.reduce((acc, item) => {
    ...
    const clienteMaisVendas = listaVendas.length > 0
        ...
        : null;
    if (elTotal)
        elTotal.textContent = totalClientes.toString();
    if (elMedia)
        elMedia.textContent = formatCurrency(faturamentoTotal);
    if (elMaiorCliente) {
