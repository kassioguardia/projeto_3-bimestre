...
function atualizarCards(relatorio) {
    ...
    if (relatorio.length === 0) {
        if (elTotal)
            elTotal.innerText = 'Nenhum dado registrado';
        if (elMedia)
            elMedia.innerText = 'Nenhum dado registrado';
        if (elMaiorCliente)
            elMaiorCliente.innerText = 'Nenhum dado registrado';
        return;
    }
    const totalClientes = new Set(relatorio.map((item) => item.id_cliente)).size;
    const faturamentoTotal = relatorio.reduce((acumulador, item) => {
        return acumulador + numeroSeguro(item.quantidade) * numeroSeguro(item.valor_unitario);
    }, 0);
    const vendasPorCliente = relatorio.reduce((acumulador, item) => {
